...
for ((n = 8; n <= 512; n *= 2)); do
    ...
    bf_time=$(./build/brute_force points.txt \
                | sed -n '/Tiempo de ejecución:/!b;n;s/^[ \t]*//;s/ ns//;p')
    dc_time=$(./build/divide_and_conquer points.txt \
                | sed -n '/Tiempo de ejecución:/!b;n;s/^[ \t]*//;s/ ns//;p')
    sbf_time=$(./build/super_brute_force points.txt \
                | sed -n '/Tiempo de ejecución:/!b;n;s/^[ \t]*//;s/ ns//;p')
    sdc_time=$(./build/super_divide_and_conquer points.txt \
                | sed -n '/Tiempo de ejecución:/!b;n;s/^[ \t]*//;s/ ns//;p')

    echo "$n,$bf_time,$dc_time,$sbf_time,$sdc_time" >>"$output_file"
done
echo -e "${G}* Resultados de benchmark guardades en archivo \"$output_file!\"${RESET}"

# Mostrando archivo csv
python plots/plot_bf_dc.py
